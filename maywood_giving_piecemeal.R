...
tmp_gift_alloc_stub_large <- sqlQuery(squibbs, "(
  SELECT DISTINCT  
...
##  Gift data
tmp_gift_stub_distinct <- sqlQuery(squibbs, "(
  SELECT DISTINCT
...
## Entity data
tmp_entity_stub <- sqlQuery(squibbs, "(
  SELECT DISTINCT
...
## Allocation data
tmp_allocation_stub <- sqlQuery(squibbs, "(
  SELECT DISTINCT
...
## Appeals data
tmp_appeals_stub <- sqlQuery(squibbs, "(
  SELECT DISTINCT
...
## Pledge data
tmp_pledge_stub <- sqlQuery(squibbs, "(
  SELECT DISTINCT
...
##  Now join them
p1_gift_w_alloc <- left_join(tmp_gift_alloc_stub_large, tmp_allocation_stub, c("GIFT_ASSOCIATED_ALLOCATION" = "ALLOCATION_CODE"), na_matches = "never")
