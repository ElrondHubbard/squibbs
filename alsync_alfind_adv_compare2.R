...
alsync <- read.csv("N:/AIS/Yerik/Data Audits/Address_et_al/vendor_test_files/FinalResults_AlumniSync_TEST_loyola/FINAL_loyola_combined_Address Date Example.csv", header=TRUE, na.strings="")
alfind <- read.csv("N:\\AIS\\Yerik\\Data Audits\\Address_et_al\\vendor_test_files\\AF_AccuData_test_file_180928_Appended_20181012\\vendor_test_file_final_180928_Appended.csv", header=TRUE, na.strings="") 
test_addr <- read.csv("N:/AIS/Yerik/Data Audits/Address_et_al/vendor_test_files/vendor_test_file_all_obs_all_var.csv", header=TRUE, na.strings="")

# Keep only the updated data from alsync n alfind
alfind2 <- subset(alfind, select=c(2,43:48))
colnames(alfind2)[1:7] <- c("ID_NUMBER","addr_af", "city_af", "state_af", "zip_af", "source_af", "dob_af")
alsync$final_zip <- paste(alsync$Final.Zip, alsync$Final.Zip...4,sep='-')
alsync2 <- subset(alsync, select=c(2:4,24:27,40,30:32,38:39))
colnames(alsync2)[4:13] <- c("addr_as", "addr2_as", "city_as", "state_as", "zip_as", "addr_date_as", "addr_result_as", "addr_src_as", "start_dt_as","asof_dt_as")

# Join original file sent to vendors, AS result, AF result
comp1 <- left_join(test_addr, alfind2, c("ID_NUMBER2" = "ID_NUMBER")) 
comp1 <- left_join(comp1, alsync2, c("F_NAME" = "First.name", "L_NAME" = "Last.name")) 
comp1$dupes <- duplicated(comp1$ID_NUMBER2)
comp1 <- subset(comp1, comp1$dupes == FALSE, select=c(1:23,25:34))
colnames(comp1)[2:3] <- c("test_id", "ID_NUMBER")
comp1$ADV_STREET <- as.character(comp1$ADV_STREET)
comp1$EDQ_STREET <- as.character(comp1$EDQ_STREET)
comp1$addr_af <- as.character(comp1$addr_af)
...
rm(comp1, comp2, alfind, alfind2, alsync, alsync2, test_addr)
...
comp_adv_as <- subset(comp2, select=c(1,17,3:5,7,11:12,16,24,30,32:37,42:45))
